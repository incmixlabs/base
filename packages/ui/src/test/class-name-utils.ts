...
export function expectNoClassToken(className: string | undefined, token: string) {
  const classTokenSet = new Set(splitClassNames(className))
  if (classTokenSet.has(token)) {
    throw new Error(`Expected class list not to contain "${token}". Received: "${className ?? ''}"`)
  }
}

export function expectNoClassTokens(className: string | undefined, tokens: readonly string[]) {
  for (const token of tokens) {
    expectNoClassToken(className, token)
  }
}
