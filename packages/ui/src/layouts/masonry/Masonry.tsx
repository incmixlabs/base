...
function getScrollParent(node: HTMLElement): HTMLElement | null {
  ...
  while (current && current !== document.documentElement) {
    ...
    if (OVERFLOW_RE.test(style.overflow + style.overflowY)) {
      return current
    }
...
function getViewportHeight(scrollParent: HTMLElement | null): number {
  if (typeof document === 'undefined') return 0
  return scrollParent ? scrollParent.clientHeight : document.documentElement.clientHeight
}
...
interface DebouncedContainerSizeOptions {
...
function useDebouncedContainerSize(options: DebouncedContainerSizeOptions) {
  ...
  // ResizeObserver on the container itself — catches parent layout shifts
  // (sidebar collapse, flexbox reflow, etc.) that window resize misses.
  React.useEffect(() => {
  ...
  }, [containerRef, setDebouncedSize])

  // Window / visual-viewport fallback for orientation changes & viewport resize
  React.useEffect(() => {
    function onResize() {
      const el = containerRef.current
      const scrollParent = el ? getScrollParent(el) : null
      setDebouncedSize({
        width: el ? el.offsetWidth : document.documentElement.clientWidth,
        height: getViewportHeight(scrollParent),
      })
    }
...
function useScroller({
  ...
  // Resolve the scroll parent once the container mounts
  React.useEffect(() => {
    const el = containerRef.current
    scrollParentRef.current = el ? getScrollParent(el) : null
  }, [containerRef])

  const readScrollTop = React.useCallback((): number => {
    const sp = scrollParentRef.current
    if (sp) return sp.scrollTop
    if (typeof globalThis.window === 'undefined') return 0
    return globalThis.window.scrollY ?? document.documentElement.scrollTop ?? 0
  }, [])

  const [scrollY, setScrollY] = useThrottle(readScrollTop, { fps, leading: true })

  const onScroll = React.useCallback(() => {
    setScrollY(readScrollTop())
  }, [readScrollTop, setScrollY])

  React.useEffect(() => {
    if (typeof globalThis.window === 'undefined') return

    const sp = scrollParentRef.current
    const target: EventTarget = sp ?? globalThis.window
...
export interface MasonryRootProps extends DivProps {
  columnWidth?: number
  columnCount?: number
  maxColumnCount?: number
...
function MasonryRoot({
  ...
  ref: forwardedRef,
  ...rootProps
}: MasonryRootProps & { ref?: React.Ref<RootElement> }) {
  ...
  const size = useDebouncedContainerSize({
    containerRef,
    defaultWidth,
    defaultHeight,
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    let offset = 0
    let container = containerRef.current as HTMLElement | null

    do {
      offset += (container as HTMLElement).offsetTop ?? 0
      container = (container as HTMLElement).offsetParent as HTMLElement | null
    } while (container)
...
export interface MasonryItemProps extends DivProps {
  asChild?: boolean
}

const MasonryItem = React.forwardRef<ItemElement, MasonryItemProps>((props, ref) => {
...
export namespace MasonryProps {
  export type Root = MasonryRootProps
  export type Item = MasonryItemProps
}
