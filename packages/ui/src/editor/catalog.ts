...
import { compositeCatalogEntryConfigs, compositeRuntimeScopeLoaders } from '@/composites/catalog'
...
import {
  ...
  tableShellPropDefs,
  tabsPropDefs,
  treeTablePropDefs as treeTableCatalogPropDefs,
} from './prop-defs'
...
const tableCatalogPropDefs = createDocsPropDefs(tablePropDefs.Root, {
  size: { typeSimple: `Responsive<${tablePropDefs.Root.size.values.map(value => `'${value}'`).join(' | ')}>` },
  layout: { typeSimple: `Responsive<${tablePropDefs.Root.layout.values.map(value => `'${value}'`).join(' | ')}>` },
})
...
const catalogRuntimeScopeLoaders = {
  ...
  table: async () => {
    const [{ Flex }, { Table }] = await Promise.all([import('@/layouts/flex/Flex'), import('@/table/basic/Table')])
    return { Flex, Table }
  },
  'table-shell': async () => {
    const [{ Flex }, { Table }, { TableShell }] = await Promise.all([
      import('@/layouts/flex/Flex'),
      import('@/table/basic/Table'),
      import('@/table/shared/TableShell'),
    ])
    return { Flex, Table, TableShell }
  },
  'tree-table': async () => {
    const [{ TreeTable }] = await Promise.all([import('@/table/tree-table')])
    return { TreeTable }
  },
  'todos-block': async () => {
    const [{ Flex }, { Text }, { TodosBlock }] = await Promise.all([
      import('@/layouts/flex/Flex'),
      import('@/typography/text/Text'),
      import('@/blocks/crud/todos/TodosBlock'),
    ])
    return { Flex, Text, TodosBlock }
  },
  'bar-chart': async () => {
...
export const tableCatalogEntry = createCatalogEntry({
  slug: 'table',
  title: 'Table',
  description: 'Structured data grid for records, metrics, and dense operational views.',
  family: 'table',
  category: 'data-display',
  sourcePath: 'packages/ui/src/table/basic/Table.tsx',
  componentName: 'Table',
  propDefs: tableCatalogPropDefs,
  overviewCode: `export default function Example() {
  return (
    <Flex direction="column" width="480px">
      <Table.Root variant="surface" size="sm">
        ...
        <Table.Body>
          <Table.Row>
            <Table.RowHeaderCell>Northwind</Table.RowHeaderCell>
...
export const tableShellCatalogEntry = createCatalogEntry({
  slug: 'table-shell',
  title: 'Table Shell',
  description: 'Container shell that pairs a table region with a persistent footer area for pagination or summaries.',
  family: 'table',
  category: 'data-display',
  sourcePath: 'packages/ui/src/table/shared/TableShell.tsx',
  componentName: 'TableShell',
  propDefs: tableShellPropDefs,
  overviewCode: `export default function Example() {
  return (
    <TableShell
      footer={
    ...
    >
      <div className="rounded-xl border p-4 text-sm text-muted-foreground">
        Place Table, TableWrapper, or InfiniteTable inside the shell body.
      </div>
    </TableShell>
  )
}`,
...
export const treeTableCatalogEntry = createCatalogEntry({
  slug: 'tree-table',
  title: 'Tree Table',
  description: 'Editable hierarchical table for nested records, grouped line items, and tree-shaped operational data.',
  family: 'table',
  category: 'data-display',
  sourcePath: 'packages/ui/src/table/tree-table/TreeTable.tsx',
  componentName: 'TreeTable',
  propDefs: treeTableCatalogPropDefs.Root,
  overviewCode: `const rows = [
  {
    id: 'services',
    name: 'Services',
    qty: 3,
    children: [
      { id: 'implementation', name: 'Implementation', qty: 2 },
      { id: 'support', name: 'Support', qty: 1 },
    ],
  },
]
...
const columns = [
  ...
  { accessorKey: 'qty', header: 'Qty', size: 120, meta: { align: 'right' } },
]
...
export const todosBlockCatalogEntry = createCatalogEntry({
  slug: 'todos-block',
  title: 'Todos Block',
  description: 'Nested task block for CRUD workflows with assignees, dates, ordering, and subtasks.',
  family: 'blocks',
  category: 'crud',
  sourcePath: 'packages/ui/src/blocks/crud/todos/TodosBlock.tsx',
  componentName: 'TodosBlock',
  propDefs: [],
  overviewCode: `export default function Example() {
  return (
    <TodosBlock
      defaultItems={[
        { id: 'todo-1', text: 'Finalize onboarding checklist', priority: 'high' },
        { id: 'todo-2', text: 'Review workspace permissions', completed: true },
      ]}
      className="max-w-xl"
    />
  )
...
export const compositeCatalogEntries = compositeCatalogEntryConfigs.map(config => createCatalogEntry(config))
...
export const localCatalogEntries: readonly CatalogEntry[] = [
  ...
  gridCatalogEntry,
  tableCatalogEntry,
  tableShellCatalogEntry,
  treeTableCatalogEntry,
  todosBlockCatalogEntry,
  barChartCatalogEntry,
  compactHorizontalChartCatalogEntry,
  mapChartCatalogEntry,
  ...compositeCatalogEntries,
]
