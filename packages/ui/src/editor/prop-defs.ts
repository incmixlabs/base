...
import { treeTablePropDefs as uiTreeTablePropDefs } from '@/table/tree-table/tree-table.props'
...
import {
  ...
  tablePropDefs as uiTablePropDefs,
...
const sharedTablePropDefs: PropDef[] = [
  {
    name: 'size',
    typeSimple: `Responsive<${enumType(uiTablePropDefs.Root.size)}>`,
    default: String(uiTablePropDefs.Root.size.default),
  },
  {
    name: 'variant',
    typeSimple: enumType(uiTablePropDefs.Root.variant),
    default: String(uiTablePropDefs.Root.variant.default),
  },
  {
    name: 'layout',
    typeSimple: `Responsive<${enumType(uiTablePropDefs.Root.layout)}>`,
    default: String(uiTablePropDefs.Root.layout.default),
  },
  { name: 'striped', typeSimple: 'boolean', default: false },
  { name: 'compact', typeSimple: 'boolean', default: false },
]
...
export const tablePropDefs = {
  Root: createDocsPropDefs(uiTablePropDefs.Root, {
    size: { typeSimple: `Responsive<${enumType(uiTablePropDefs.Root.size)}>` },
    layout: { typeSimple: `Responsive<${enumType(uiTablePropDefs.Root.layout)}>` },
  }),
  Row: createDocsPropDefs(uiTablePropDefs.Row),
  Cell: createDocsPropDefs(uiTablePropDefs.Cell),
} as const
export const tableWrapperPropDefs: PropDef[] = [
  { name: 'data', typeSimple: 'TableWrapperData', required: true },
  ...sharedTablePropDefs,
  { name: 'expandAll', typeSimple: 'boolean', default: false },
  {
    name: 'groupBy',
    typeSimple: 'readonly string[]',
    description: 'Groups rows by column ids and renders grouped rows through the tree-capable path.',
  },
  {
    name: 'defaultGroupBy',
    typeSimple: 'readonly string[]',
    description: 'Initial uncontrolled grouping column ids.',
  },
  {
    name: 'shape',
    typeSimple: "'flat' | 'tree'",
    description: 'Controls whether nested rows render as a flat list or expandable tree.',
  },
  {
    name: 'defaultShape',
    typeSimple: "'flat' | 'tree'",
    description: 'Initial uncontrolled table shape when shape is not controlled.',
  },
  {
    name: 'editable',
    typeSimple: 'boolean',
    default: true,
    description: 'Enables editable behavior when onCellEdit is provided.',
  },
  { name: 'defaultCellMode', typeSimple: "'read' | 'edit'", default: 'read' },
  { name: 'cellMode', typeSimple: "'read' | 'edit'" },
  { name: 'onCellModeChange', typeSimple: "(mode: 'read' | 'edit') => void" },
  { name: 'allowedCellModes', typeSimple: "readonly ('read' | 'edit')[]" },
  {
    name: 'onCellEdit',
    typeSimple: 'TableCellEditCommitHandler<TableWrapperRow, TableWrapperCellValue>',
    description: 'Commit handler required to persist edits.',
  },
  { name: 'onRowSelect', typeSimple: '(row: TableWrapperRow) => void' },
  {
    name: 'rowActions',
    typeSimple: 'TableWrapperRowActions',
    description: 'Optional duplicate and remove command callbacks rendered with shared row actions.',
  },
  { name: 'renderCell', typeSimple: '(row, column, defaultRender) => ReactNode' },
  { name: 'renderRow', typeSimple: '(row, defaultRender) => ReactNode' },
  { name: 'meta', typeSimple: 'ReactNode' },
  { name: 'toolbarActions', typeSimple: 'ReactNode' },
  { name: 'totalRows', typeSimple: 'number' },
  { name: 'filteredRows', typeSimple: 'number' },
  { name: 'activeFilterCount', typeSimple: 'number' },
  { name: 'onReset', typeSimple: '() => void' },
]
export const treeTablePropDefs = {
  Root: createDocsPropDefs(uiTreeTablePropDefs.Root, {
    columns: {
      typeSimple: 'TreeTableColumnDef<TData>[]',
      description:
        'Low-level TanStack-compatible columns. Prefer shared TableColumnDef + toTanStackTableColumnDefs for reusable schemas.',
    },
    data: { typeSimple: 'TData[]' },
    defaultSorting: { typeSimple: 'SortingState' },
    defaultExpanded: { typeSimple: 'ExpandedState' },
    size: { typeSimple: enumType(uiTreeTablePropDefs.Root.size) },
    variant: { typeSimple: enumType(uiTreeTablePropDefs.Root.variant) },
    color: { typeSimple: 'Color' },
    children: { typeSimple: 'ReactNode' },
  }),
  Toolbar: createDocsPropDefs(uiTreeTablePropDefs.Toolbar),
  Content: createDocsPropDefs(uiTreeTablePropDefs.Content),
} as const
export const tableShellPropDefs: PropDef[] = [
  { name: 'footer', typeSimple: 'ReactNode' },
  { name: 'children', typeSimple: 'ReactNode' },
  { name: 'className', typeSimple: 'string' },
]
