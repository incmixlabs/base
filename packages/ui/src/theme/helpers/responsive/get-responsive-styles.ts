import type { Responsive, Union } from '@/theme/props/prop-def'
import { hasOwnProperty as hasOwn } from '../has-own-property'
...
function getResponsiveClassNames({
  ...
  if (isResponsiveObject(value)) {
    const object = value as Record<string, string | undefined>

    for (const bp in object) {
      // Make sure we are not iterating over keys that aren't breakpoints
      if (!hasOwn(object, bp) || !breakpoints.has(bp)) {
        continue
      }
...
function getResponsiveCustomProperties({
  ...
  if (isResponsiveObject(value)) {
    ...
    for (const bp in object) {
      ...
      if (value === undefined || propValues.includes(value)) {
        continue
      }
