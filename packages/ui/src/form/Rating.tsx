...
import { HORIZONTAL_ARROW_KEYS, isActivationKey, KEYBOARD_KEYS, VERTICAL_ARROW_KEYS } from '@/lib/keyboard-keys'
...
const MAP_KEY_TO_FOCUS_INTENT: Record<string, FocusIntent> = {
  [KEYBOARD_KEYS.arrowLeft]: 'prev',
  [KEYBOARD_KEYS.arrowUp]: 'prev',
  [KEYBOARD_KEYS.arrowRight]: 'next',
  [KEYBOARD_KEYS.arrowDown]: 'next',
  [KEYBOARD_KEYS.home]: 'first',
  [KEYBOARD_KEYS.end]: 'last',
}

function getDirectionAwareKey(key: string, dir?: Direction) {
  if (dir !== 'rtl') return key
  return key === KEYBOARD_KEYS.arrowLeft
    ? KEYBOARD_KEYS.arrowRight
    : key === KEYBOARD_KEYS.arrowRight
...
function getFocusIntent(event: React.KeyboardEvent<ItemElement>, dir?: Direction, orientation?: Orientation) {
  ...
  if (orientation === 'vertical' && HORIZONTAL_ARROW_KEYS.includes(key as (typeof HORIZONTAL_ARROW_KEYS)[number])) {
    return undefined
  }
      ...
      <RatingContext.Provider value={contextValue}>
        <FocusContext.Provider value={focusContextValue}>
          <RootPrimitive
            ...
            className={cn(
              ratingBase,
              ratingColorVariants[color],
              ratingGapVariants[size],
...
function RatingItem(props: RatingItemProps) {
  ...
  const selectedItemValue = step < 1 ? Math.ceil(value) : value
  const isChecked = selectedItemValue === itemValue
  ...
  const wasAddedRef = React.useRef(false)
  ...
  const onFocus = React.useCallback(
    ...
    [focusContext, itemId, activationMode, isDisabled, isReadOnly, value, itemValue, step, store, propsRef],
  )
  ...
  const onKeyDown = React.useCallback(
    (event: React.KeyboardEvent<ItemElement>) => {
      ...
      if (isActivationKey(event.key) && activationMode === 'manual') {
