...
import { calendarArrowKeyMoves, isCalendarArrowKey } from './date-next-keyboard'
...
import { useDateNextCalendar } from './useDateNextCalendar'
...
export interface DateNextCalendarPanelProps {
  value?: Date
...
/**
 * Shared calendar panel for date-next components.
 * Renders month navigation + weekday labels + day grid.
 * Used by DatePickerNext, AppointmentPickerNext, MiniCalendarNext,
 * CalendarWithPricingNext, and CalendarWithPresetsNext.
 */
export function DateNextCalendarPanel({
  value,
  onChange,
  minValue,
  maxValue,
  ...
  return (
    <div className={cn('space-y-2', dateNextElementRadiusStyles[radius], className)}>
      {headerContent}
      <div
        className={cn(
          datePickerCalendarWeekdayGrid,
