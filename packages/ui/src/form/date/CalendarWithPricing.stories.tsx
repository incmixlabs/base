...
const buildMonthPrices = (referenceDate: Date, toPrice: (date: Date, index: number) => DayPrice): DayPrice[] => {
  const monthStart = startOfMonth(referenceDate)
  const monthEnd = endOfMonth(referenceDate)
  return eachDayOfInterval({ start: monthStart, end: monthEnd }).map(toPrice)
}

const generatePrices = (basePrice = 100, seed = 42, referenceDate = STORY_REFERENCE_DATE): DayPrice[] =>
...
export const WeekStartsMonday: Story = {
  render: args => {
    ...
    return (
      <div className="w-[400px]">
        <CalendarWithPricing {...args} value={date} onChange={setDate} prices={prices} weekStartsOn={1} />
      </div>
    )
  },
}
...
export const Disabled: Story = {
  render: args => {
    ...
    return (
      <div className="w-[400px]">
        <CalendarWithPricing {...args} value={STORY_REFERENCE_DATE} prices={prices} isDisabled />
      </div>
    )
  },
}
