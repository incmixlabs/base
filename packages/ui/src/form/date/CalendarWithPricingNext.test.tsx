...
function buildPrices(overrides?: Partial<DayPriceNext>[]): DayPriceNext[] {
  const base: DayPriceNext[] = Array.from({ length: 31 }, (_, i) => ({
...
describe('CalendarWithPricingNext', () => {
  it(
    'renders price labels for days in the current month',
    () => {
      const prices = buildPrices()
      act(() => {
        render(<CalendarWithPricingNext value={march2026} prices={prices} />)
      })
  ...
  it('calls onChange when a day with price is clicked', async () => {
    ...
    act(() => {
      render(<CalendarWithPricingNext value={march2026} onChange={onChange} prices={prices} />)
    })
  ...
  it('uses custom currency symbol', () => {
    ...
    act(() => {
      render(<CalendarWithPricingNext value={march2026} prices={prices} currency="€" />)
    })
  ...
  it('marks selected date with data-selected', () => {
    ...
    act(() => {
      render(<CalendarWithPricingNext value={new Date(2026, 2, 15)} prices={prices} />)
    })
  ...
  it('disables all interactions when isDisabled is true', () => {
    ...
    act(() => {
      render(<CalendarWithPricingNext value={march2026} onChange={onChange} prices={prices} isDisabled />)
    })
