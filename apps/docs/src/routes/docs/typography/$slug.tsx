...
import { loadTypographyDocsEntry } from '../../../lib/typography-docs-registry'

export const Route = createFileRoute('/docs/typography/$slug')({
  component: TypographyDocsPageRoute,
})

function TypographyDocsPageRoute() {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        const loadedEntry = await loadTypographyDocsEntry(slug)
